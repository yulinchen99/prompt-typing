...
def main():
    ...
    parser.add_argument('--random_verbalizer', action='store_true', default=False)
    ...
    if args.model == "baseline":
        ...
        # tag_mapping = dict(zip(tag_mapping.keys(), random.shuffle(list(tag_mapping.values()))))
        mapped_tag_list = [tag_mapping[t] for t in tag_list]
        if args.random_verbalizer:
            random.shuffle(mapped_tag_list)
            tag_mapping = dict(zip(tag_list, mapped_tag_list))
    ...
    if not args.test_only:
        ...
        for i in range(epoch):
            ...
            for data in tqdm(train_dataloader, desc=f"Train Epoch {i+1}"):
                ...
                if isinstance(Loss, MultiLabelLoss):
